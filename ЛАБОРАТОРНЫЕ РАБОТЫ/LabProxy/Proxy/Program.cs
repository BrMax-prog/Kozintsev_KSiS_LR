...
namespace Proxy
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine($"Запуск прокси-сервера на {ProxyIp}:{ProxyPort}...");

            IPAddress ipAddress = IPAddress.Parse(ProxyIp);
            TcpListener listener = new TcpListener(ipAddress, ProxyPort);
            listener.Start();

            Console.WriteLine("Прокси работает. Настройте браузер на 127.0.0.2:9000\n");
        ...
        static async Task TransferAndLogResponse(NetworkStream serverStream, NetworkStream browserStream, string fullUrl)
        {
            ...
            while ((bytesRead = await serverStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
            {
                if (isFirstChunk)
                {
                    ...
                    Match match = Regex.Match(responseText, @"^HTTP/\d\.\d\s+(\d{3})");
                    string statusCode = match.Success ? match.Groups[1].Value : "???";
                    if (statusCode == "200")
                    {
                        statusCode += " OK";
                    }
